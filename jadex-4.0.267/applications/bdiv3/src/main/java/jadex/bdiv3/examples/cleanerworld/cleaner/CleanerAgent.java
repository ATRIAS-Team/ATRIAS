...
import jadex.bdiv3.examples.cleanerworld.world.*;
...
@Plans(
{
	...
	@Plan(trigger=@Trigger(goals=CleanerAgent.PerformPatrol.class), body=@Body(NeuralNetworkPlan.class)),
	...
	//@Plan(trigger=@Trigger(goals=CleanerAgent.PerformPatrol.class), priority=1, body=@Body(NeuralNetworkPlan.class))
})
...
//				System.out.println("Error when updating vision! "+event.getGoal());
	...
	//@AgentBody
	@OnStart
	public void body() throws ModelException, TranslateException, IOException {
		...
		//Classifications classifications = NeuralNetworkUtil.predict();
