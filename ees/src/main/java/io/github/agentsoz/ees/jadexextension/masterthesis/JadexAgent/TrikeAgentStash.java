...
//        System.out.println("AgentID: " + agentID + " - EvaluateDecisionTask at simtime " + JadexModel.simulationtime);
    ...
    List<Integer> tripsToScheduleSizes = new ArrayList<>();
    ...
    public Integer selectNextAction(Integer position){
        ...
        if (decisionTaskList.get(position).getStatus().equals("new")){
            ...
            tripsToScheduleSizes.add(tripList.stream()
            ...
            try {
                ...
                if (mode.equals("GENETIC")) {
                    GeneticScheduler geneticScheduler = new GeneticScheduler(initConfigurations());
...
//                    System.out.println("Agent " + agentID + " new trip list: " + tripList.stream().map(t -> t.getTripID()).collect(Collectors.toList()));
                ...
                System.out.println("SCHEDULE RESULT " + tripList.stream().map(t -> t.getTripID()).collect(Collectors.toList()) + " at " + JadexModel.simulationtime);
//                    System.out.println("TRIP LIST SIZES: " + tripsToScheduleSizes);
                tripIDList.add("1");
            } catch (Exception e) {
...
//            estimateBatteryAfterTIP();
    ...
    public Config initConfigurations() {
        ...
        config.setCurrentTrip(currentTrip);
        config.setSimtime(JadexModel.simulationtime);
...
//                logCheck.add(tripList.get(0).getTripID());
    ...
    void currentTripStatus() {
        if (currentTrip.size() > 0){
            ...
            System.out.println("AgentID: " + agentID + " triplist sizes: " + tripsToScheduleSizes);
        }
...
//    }
    ...
    public boolean customerMissOld(Trip trip) {
        ...
        LocalDateTime currentSimtime = getCurrentSimulationTimeAsDate();
        LocalDateTime customerIsMissedAt = vaTime.plusMinutes((long) (THETA / 60.0));
        if (customerIsMissedAt.isAfter(currentSimtime) && ALLOW_CUSTOMER_MISS){
...
//        estimateBatteryAfterTIP();
    ...
    {
        System.out.println("AgentID: " + agentID + " sendDriveToToAdc at " + JadexModel.simulationtime);
