...
public class JadexModel implements BDIServerInterface, ModelInterface, DataClient, EnvironmentActionInterface, Runnable {
    ...
    //Robustheit
    public static boolean disruptionToggle;
    public static List<DisruptionInjector> toDisruptAgents;
    private boolean signalSent = false;


    public JadexModel(Map<String, String> optsü, DataServer dataServer, QueryPerceptInterface qpi) {
    ...
    public AgentDataContainer takeControl(double time, AgentDataContainer inAdc) {
        ...
        if (inAdc != null) {
            ...
            //Robustheit
            if(disruptionToggle && !signalSent) {
                //disruption component
                SimSensoryInputBroker.toDisruptAgents = this.toDisruptAgents;
                signalSent = true;
            }
            run();
