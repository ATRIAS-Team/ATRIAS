...
public class TestV3TrikeAgent1 {
    ...
    @Belief
    protected String resultfromMATSIM ="false";
    ...
    @Belief
    private List<ActionContent> ActionContentList = new ArrayList<>();

    @Belief
    private List<PerceptContent> PerceptContentList = new ArrayList<>();
    ...
    class SendTooutAdc {
        ...
        @GoalCreationCondition(factadded = "tripList") //
        public SendTooutAdc() {
    ...
    public void sendtoOutAdc()
    {SendtoAgentDataContainer();}
 ...
 */
    ...
    public void UpdateSensory() {
        ...
        resultfromMATSIM = "false";
    }
    @Goal(recur = true,recurdelay = 3000)
    class InformwhenFinished {
        // when the DRIVE_TO Action is successful, trikeagent will become newly active agents in this cycle be added to the list of activeagent in the sensoryAgent
        //In this BDI-MATSIM cycle, these active TrikeAgent needs to process the input to either execute new trip or wait
        // for new trip and inform the SimSensory Agent. The BDI cycle is finished when SimSensory receives information from all of these
        //newlyactiveAgents.
        @GoalCreationCondition(beliefs = "activestatus") //
        public InformwhenFinished() {
            System.out.println("I inform SimSensoryInputBroker when I am done with this cycle");
        }
        @GoalTargetCondition
    ...
    //should take the first trip from the trip list
    @Plan(trigger = @Trigger(goalfinisheds = PerformSIMReceive.class))
    public void ExecuteandInform()
    {	if (activestatus == true)
    {
        SendtoAgentDataContainer();
        if (currentAreaAgent == "AreaAgent1") {
            AreaAgent1.ActiveAgentList.remove(agentID);
        }

        if (currentAreaAgent == "AreaAgent2") {
            AreaAgent2.ActiveAgentList.remove(agentID);
        }
    }
    ...
    public String getRandomAreaAgent() {
        //List <Class> AreaAgentClass = Arrays.asList(AreaAgent1.class,AreaAgent2.class);
