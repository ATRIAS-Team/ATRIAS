...
//})
//
//public class TrikeAgentThreadsWithExecutorService implements SendtoMATSIM{
//
...
//    private List<Trip> tempTripList = new ArrayList<>();
//
...
//                    List<Trip> result = geneticScheduler.start(tripList, ITERS, RUNS);
//                    List<String> idsOfAp = tripList.stream().map(t -> {
//                        if (t.getTripID().startsWith("AP")) {
//                            return t.getTripID();
//                        }
//                        return null;
//                    }).collect(Collectors.toList());
//
//                    // may contain duplicate chargingtrips
//                    Iterator<Trip> it = result.iterator();
//                    while (it.hasNext()) {
//                        Trip curr = it.next();
//                        // trip wurde aus der triplist genommen während geschedulet wurde
//                        // TODO mit Unique chargingIds kann dieselbe Funktion verwendet werden
//                        if (curr.getTripID().startsWith("AP") && !idsOfAp.contains(curr.getTripID())) {
//                            it.remove();
//                        }
//                    }
//
...
//                    int initialPopulationSize = (totalPermutations / 10) < 25 ? 25 : Math.min(totalPermutations / 10, 500);
//
...
//                    } else {
//                        // customer trip
//                        List<Trip> listTrip = trips.stream().filter(t -> t.getTripID() == gene.getId()).collect(Collectors.toList());
//                        if (listTrip.size() > 0) {
//                            result.add(listTrip.get(0));
//                        }
//                    }
//                }
//
...
//
//                // ToDo: Mit while schleife befüllen bei 250 init size entstehen nur 17 unterschiedliche elemente :D
...
//        }
//
//        // if there is one elem that isnt null there will be a charging gene with charging time
//        private boolean containsChargingTimes(Chromosome chromosome) {
//            List<Gene> chargingGenes = chromosome.getChargingChromosome();
//            for (int i = 0; i < chargingGenes.size(); i++) {
...
//                    ? Location.distanceBetween(gene.getStart(), gene.getEnd())
//                    : 0.0;
//        }
//
...
//            final List<Gene> customerCopy = new ArrayList<>(this.customerChromosome);
//            List<Gene> chargingCopy = new ArrayList<>(this.chargingChromosome);
//            List<Gene> mutated = geneticUtils.mutateChargingTimes(chargingCopy);
...
//
//                    gene.setChargingTime(Double.valueOf(randomInt));
//                    result.set(indexToSet, gene);
//                }
//                return result;
//            } catch (Exception e) {
//                System.out.println("Caught exception");
