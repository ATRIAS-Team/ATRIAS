...
public class TrikeAgent implements SendtoMATSIM{
    ...
    Double THETA = 780.0; //allowed waiting time for customers.
    ...
    public void setToDisruptAgents(List<DisruptionInjector> toDisruptAgents) {
        this.toDisruptAgents = toDisruptAgents;
        this.signalDisrupt = true;
    }
    ...
    public void NewChargingTrip() {
        {
            //Robustheit: Note! For the master thesis, the call of the function was put here, as this plan is triggered
            // regularly. In the future, this can be packed into a separate plan. For now it serves its purpose.
            if(signalDisrupt) {
                signalDisruption();
            }
            if (estimateBatteryAfterTIP.get(0) < CHARGING_THRESHOLD && chargingTripAvailable.equals("0") && !breakdownChargingBlocker){
    ...
    //Robustheit
    private void signalDisruption(){

        //Check whether toDisruptAgents contains the current agent.
        if(DisruptionInjectorUtils.listContainsAgent(toDisruptAgents,this.agentID)){
            DisruptionInjector currentAgent = DisruptionInjectorUtils.findAgentID(toDisruptAgents,this.agentID);
            //When the time is right, the disrupt is triggered
            if(currentAgent.getDisruptionTime() <= JadexModel.simulationtime) {
                if(currentAgent.getDisruptionType().equals("CommunicationDisruption") ) {
                    if(!batteryLoadingBeforeEmergencyTrip) {
                        ...
                        //Creation of the Emergency-Job that will be processed to a Trip.
                        if (!emergencyTrip) {
                    ...
                    if(!trikeIsBroken) {
                        ...
                        //Set current ongoing trip to "failed".
                        if (currentTrip.size() > 0) {
                            prepareLog(currentTrip.get(0), "0.0", "0.0", "false", "0.0");
                            currentTrip.get(0).setProgress("Failed");
                        }

                        //Tasks are delegated due to failure of trike if CNP is active.
                        //TODO: only delegate tasks that originate from this trike
                        if (decisionTaskList.size() > 0 && CNP_ACTIVE) {
                    ...
                    System.out.println("Wrong Disruption-Type");
                }
            }
        }
    ...
    public Integer selectNextAction(Integer position){
        ...
        //Robustheit
            ...
            tripIDList.add("1");
            chargingTripAvailable = "1";
            batteryLoadingBeforeEmergencyTrip = true;
            ...
            decisionTaskList.get(position).setTimeStampWFN(JadexModel.simulationtime);
            decisionTaskList.get(position).setStatus("waitingForNeighbours");
            /*
            if(toDisruptAgents != null){
                sendMessageAreaAgent("area:0", "request", "callForNeighbours", values);
            }else{
            ...
            */
            ...
            for (int i=0; i<decisionTaskList.get(position).getUTScoreList().size(); i++){
                ...
                if(tag.equals("AcceptProposal")){
                    ...
                    decisionTaskList.get(position).setTimeStampWFC(JadexModel.simulationtime);
            ...
            decisionTaskList.get(position).setTimeStampWFM(JadexModel.simulationtime);
        ...
        //Robustheit
            ...
            if (decisionTaskList.get(position).getTimeStampWFC()+ TIMEOUT_THRESHOLD_WFC <= JadexModel.simulationtime) {
        ...
        //Robustheit
            ...
            if(decisionTaskList.get(position).getTimeStampWFM()+ TIMEOUT_THRESHOLD_WFM <= JadexModel.simulationtime){
                decisionTaskList.get(position).setStatus("notAssigned");
                changes += 1;
            }
        }
    ...
    Double calculateUtility(DecisionTask newTask){
        ...
        //Robustheit: If trike is broken, set utilityscore to 0.0
        if(trikeIsBroken){
            utillityScore = 0.0;
        }
        ...
        if (agentID != null) // only react if the agentID exists
        {
            if (SimIDMapper.NumberSimInputAssignedID.size() == JadexModel.SimSensoryInputBrokernumber) // to make sure all SimInputBroker also receives its ID so vehicle agent could choose one SimInputBroker ID to register
                if (sent == false) { // to make sure the following part only executed once
                    ...
                    else if (agentID.equals("33")){
                        agentLocation = new Location("", 330562.816, 4691951.597);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("34")){
                        agentLocation = new Location("", 328339.214, 4686309.690);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("35")){
                        agentLocation = new Location("", 328914.473, 4689722.320);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("36")){
                        agentLocation = new Location("", 330121.529, 4685666.860);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("37")){
                        agentLocation = new Location("", 332336.418, 4689036.823);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("38")){
                        agentLocation = new Location("", 331374.527, 4689493.447);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("39")){
                        agentLocation = new Location("", 328554.314, 4691050.880);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("40")){
                        agentLocation = new Location("", 328408.376, 4690291.468);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("41")){
                        agentLocation = new Location("", 329384.471, 4689911.517);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("42")){
                        agentLocation = new Location("", 330748.810, 4689896.080);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("43")){
                        agentLocation = new Location("", 330841.656, 4691402.006);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("44")){
                        agentLocation = new Location("", 329939.713, 4691704.073);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("45")){
                        agentLocation = new Location("", 329987.630, 4692072.121);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("46")){
                        agentLocation = new Location("", 328400.333, 4692736.850);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("47")){
                        agentLocation = new Location("", 330253.355, 4690729.156);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("48")){
                        agentLocation = new Location("", 329546.587, 4690679.550);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("49")){
                        agentLocation = new Location("", 330669.230, 4692572.359);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("50")){
                        agentLocation = new Location("", 328664.301, 4692715.827);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("51")){
                        agentLocation = new Location("", 327443.488, 4687886.430);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("52")){
                        agentLocation = new Location("", 329457.517, 4686765.274);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("53")){
                        agentLocation = new Location("", 328575.172, 4687419.778);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("54")){
                        agentLocation = new Location("", 326951.879, 4688746.432);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("55")){
                        agentLocation = new Location("", 329914.806, 4685005.340);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("56")){
                        agentLocation = new Location("", 329210.623, 4685766.426);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("57")){
                        agentLocation = new Location("", 329146.197, 4687183.842);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("58")){
                        agentLocation = new Location("", 330653.494, 4688786.536);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("59")){
                        agentLocation = new Location("", 331421.452, 4689573.203);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("60")){
                        agentLocation = new Location("", 331002.342, 4692271.983);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("61")){
                        agentLocation = new Location("", 332789.342, 4692468.604);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("62")){
                        agentLocation = new Location("", 329070.509, 4686693.813);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("63")){
                        agentLocation = new Location("", 330279.417, 4686925.322);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("64")){
                        agentLocation = new Location("", 329864.114,4690514.672);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("65")){
                        agentLocation = new Location("", 330561.816, 4691949.597);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("66")){
                        agentLocation = new Location("", 328337.214, 4686307.690);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("67")){
                        agentLocation = new Location("", 328912.473, 4689720.320);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("68")){
                        agentLocation = new Location("", 330119.529, 4685664.860);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("69")){
                        agentLocation = new Location("", 332334.418, 4689034.823);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("70")){
                        agentLocation = new Location("", 331372.527, 4689491.447);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("71")){
                        agentLocation = new Location("", 328552.314, 4691048.880);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("72")){
                        agentLocation = new Location("", 328406.376, 4690289.468);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("73")){
                        agentLocation = new Location("", 329382.471, 4689909.517);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("74")){
                        agentLocation = new Location("", 330746.810, 4689894.080);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("75")){
                        agentLocation = new Location("", 330839.656, 4691400.006);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("76")){
                        agentLocation = new Location("", 329937.713, 4691702.073);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("77")){
                        agentLocation = new Location("", 329985.630, 4692070.121);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("78")){
                        agentLocation = new Location("", 328398.333, 4692734.850);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("79")){
                        agentLocation = new Location("", 330251.355, 4690727.156);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("80")){
                        agentLocation = new Location("", 329544.587, 4690677.550);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("81")){
                        agentLocation = new Location("", 330667.230, 4692570.359);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("82")){
                        agentLocation = new Location("", 328662.301, 4692713.827);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("83")){
                        agentLocation = new Location("", 327441.488, 4687884.430);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("84")){
                        agentLocation = new Location("", 329455.517, 4686763.274);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("85")){
                        agentLocation = new Location("", 328573.172, 4687417.778);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("86")){
                        agentLocation = new Location("", 326949.879, 4688744.432);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("87")){
                        agentLocation = new Location("", 329912.806, 4685003.340);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("88")){
                        agentLocation = new Location("", 329208.623, 4685764.426);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("89")){
                        agentLocation = new Location("", 329144.197, 4687181.842);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("90")){
                        agentLocation = new Location("", 330651.494, 4688784.536);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("91")){
                        agentLocation = new Location("", 331419.452, 4689571.203);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("92")){
                        agentLocation = new Location("", 331000.342, 4692269.983);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("93")){
                        agentLocation = new Location("", 332787.342, 4692466.604);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("94")){
                        agentLocation = new Location("", 329068.509, 4686691.813);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("95")){
                        agentLocation = new Location("", 330277.417, 4686923.322);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("96")){
                        agentLocation = new Location("", 329868.114, 4690518.672);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("97")){
                        agentLocation = new Location("", 330565.816, 4691956.597);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("98")){
                        agentLocation = new Location("", 328342.214, 4686312.690);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("99")){
                        agentLocation = new Location("", 328917.473, 4689725.320);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("100")){
                        agentLocation = new Location("", 330124.529, 4685669.860);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("101")){
                        agentLocation = new Location("", 332339.418, 4689039.823);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("102")){
                        agentLocation = new Location("", 331377.527, 4689496.447);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("103")){
                        agentLocation = new Location("", 328557.314, 4691053.880);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("104")){
                        agentLocation = new Location("", 328411.376, 4690294.468);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("105")){
                        agentLocation = new Location("", 329387.471, 4689914.517);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("106")){
                        agentLocation = new Location("", 330751.810, 4689899.080);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("107")){
                        agentLocation = new Location("", 330844.656, 4691405.006);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("108")){
                        agentLocation = new Location("", 329942.713, 4691707.073);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("109")){
                        agentLocation = new Location("", 329990.630, 4692075.121);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("110")){
                        agentLocation = new Location("", 328403.333, 4692739.850);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("111")){
                        agentLocation = new Location("", 330256.355, 4690732.156);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("112")){
                        agentLocation = new Location("", 329549.587, 4690682.550);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("113")){
                        agentLocation = new Location("", 330672.230, 4692575.359);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("114")){
                        agentLocation = new Location("", 328667.301, 4692718.827);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("115")){
                        agentLocation = new Location("", 327446.488, 4687889.430);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("116")){
                        agentLocation = new Location("", 329460.517, 4686768.274);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("117")){
                        agentLocation = new Location("", 328578.172, 4687422.778);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("118")){
                        agentLocation = new Location("", 326954.879, 4688749.432);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("119")){
                        agentLocation = new Location("", 329917.806, 4685008.340);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("120")){
                        agentLocation = new Location("", 329213.623, 4685769.426);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("121")){
                        agentLocation = new Location("", 329149.197, 4687186.842);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("122")){
                        agentLocation = new Location("", 330656.494, 4688789.536);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("123")){
                        agentLocation = new Location("", 331424.452, 4689576.203);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("124")){
                        agentLocation = new Location("", 331005.342, 4692274.983);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("125")){
                        agentLocation = new Location("", 332792.342, 4692471.604);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("126")){
                        agentLocation = new Location("", 329073.509, 4686696.813);
                        sendAreaAgentUpdate("register");
                    }
                    else if (agentID.equals("127")){
    ...
    void newCurrentTrip(){
        ...
        if(currentTrip.size()==0 && tripList.size()>0 && !trikeIsBroken){
